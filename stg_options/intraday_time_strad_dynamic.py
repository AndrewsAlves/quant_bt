...
###########################################
### TIME BASED STRADDLE WITH DYNAMIC SL USING ATR< BOLLINGER  ##########
###########################################
...
#%%
bnfResampled['atr'] = bnfResampled.ta.atr()
...
def placeOrder(tickerSymbol, tickerDf, datetime, atrMultiplied, qty, tradeType, tradeObj = None, adaptivePs = False) :
    if tradeObj == None : 
        tradeObj = bt.Trade()
    ...
    if tickerDf is None:
        ...
        if datetime in tickerDf.index :
            ...
            sl = entryprice + (atrMultiplied * 0.5)
            slPercentage.append(round((atrMultiplied * 0.5) / (entryprice / 100), 2))
            if adaptivePs : 
...
for i, row in tqdm(bnfResampled.iterrows(), desc = "Backtesting", total = bnfResampled.shape[0]): 
    ...
    lowP = row["Low"]
    closeP = row["Close"]
    atrMultiplied = row['atr_multiplied']
    expiryday = False
    ...
    if datentime.time().hour == time_hour and datentime.time().minute == time_min :
        ...
        """ CE EXECUTION """
        ce_entry = placeOrder(tickerSymbolCE, CE_df, datentime, atrMultiplied, qty, "short", adaptivePs = True)
                
        """ PE EXECUTION """
        pe_entry = placeOrder(tickerSymbolPE, PE_df, datentime, atrMultiplied, ce_entry.qty, "short", adaptivePs=False)
        ...
        priceTrackerDf2[pe_entry.tradeId] = PE_df
        priceTrackerDf2[ce_entry.tradeId] = CE_df
    ...
    if datentime >= datentimeCheck :
        for tradeId in list(positions) :
            ...
            if trade.orderStatus == 0 and tradeId in priceTrackerDf : 
                placeOrder(trade.symbol, priceTrackerDf[tradeId], datentime, atrMultiplied, qty, "short", tradeObj = trade, adaptivePs= True)
